...
function setup({platform='Win32',maxTouchPoints=0,fallback=false,pitch=40}={}){
 ...
 const map={fallback,getContainer:()=>container,getCanvas:()=>canvas,getBearing:()=>state.bearing,getZoom:()=>state.zoom,getPitch:()=>state.pitch,getMinZoom:()=>10.5,getMaxZoom:()=>19,getMinPitch:()=>0,getMaxPitch:()=>70,stop(){},on:(n,f)=>events.set(n,f),off:n=>events.delete(n)};
 map[fallback?'easeTo':'jumpTo']=o=>{calls.push(o);Object.assign(state,o);};
 const controls=installMapGestures(map);
 const fire=(target,type,props={})=>{const e=new Event(type,{cancelable:true});for(const [k,v]of Object.entries(props))Object.defineProperty(e,k,{value:v});target.dispatchEvent(e);return e;};
 return {win,doc,container,canvas,calls,events,state,controls,fire};
}
test('middle drag orbits bearing/pitch, captures pointer, releases outside map and restores cursor',()=>{
 ...
 x.fire(x.win,'pointermove',{pointerId:4,buttons:4,clientX:150,clientY:0});assert.deepEqual(x.calls.at(-1),{bearing:30,pitch:70});
...
test('2D rotation stays flat and Canvas uses immediate compatible camera updates',()=>{
 for(const options of [{pitch:0},{fallback:true}]){const x=setup(options);x.fire(x.container,'pointerdown',{button:1,pointerId:1,clientX:0,clientY:0});x.fire(x.win,'pointermove',{pointerId:1,buttons:4,clientX:-50,clientY:100});assert.equal(x.calls[0].bearing,-10);assert.equal(x.calls[0].pitch,undefined);if(options.fallback)assert.equal(x.calls[0].duration,0);}
});
...
test('middle autoscroll blocked, unrelated pointers ignored, lost buttons/blur/cancel terminate and teardown removes listeners',()=>{
 const x=setup({platform:'MacIntel'});for(const type of ['mousedown','auxclick'])assert.equal(x.fire(x.container,type,{button:1}).defaultPrevented,true);
