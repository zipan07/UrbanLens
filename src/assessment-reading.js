export function installReadingView(map){
 ...
 const set=on=>{document.body.classList.toggle('reading-wide',on);button.setAttribute('aria-expanded',String(on));button.setAttribute('aria-label',on?'返回地图视图':'展开评估阅读视图');button.title=on?'返回地图视图':'向左展开 · 专注阅读评估';button.innerHTML=on?'›<span>返回地图</span>':'‹';if(on){panel.classList.add('is-open');document.querySelector('#mobile-panel').setAttribute('aria-expanded','true');if(document.body.classList.contains('is-presenting')){document.body.classList.add('present-panel');document.querySelector('#presentation-panel').setAttribute('aria-pressed','true');}}requestAnimationFrame(()=>map.resize?.());};
