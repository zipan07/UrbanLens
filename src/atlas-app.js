...
const state={theme:'day',mode:'3d',terrain:true,schematic:true,district:'all',layers:{},selected:null,imported:EMPTY(),measure:[],measuring:false,ready:false};
...
async function json(path){if(/^data\/(services|boundary|buildings|roads|land|water|pois)\.geojson$/.test(path))return JSON.parse(new TextDecoder().decode(await packedBytes(path.split('/')[1].split('.')[0])));const r=await fetch(`${base}${path}?v=9`,{signal:AbortSignal.timeout(30000)});if(!r.ok)throw new Error(`数据文件加载失败（${r.status}）：${path}`);return r.json();}
...
function refreshStateSources(){sourceData('selected',state.selected?{type:'FeatureCollection',features:[state.selected]}:EMPTY());sourceData('imported',state.imported);sourceData('districtFocus',{type:'FeatureCollection',features:data.boundary.features.filter(f=>String(f.properties.division_code)===state.district)});updateMeasure();studio?.mapLayers();}
...
function setDistrict(code){ready(()=>{state.district=code||'all';const district=manifest.districts?.find(d=>String(d.divisionCode)===state.district),b=district?.bbox||manifest.boundary.bbox;detailsUI?.close();shellUI?.stopOrbit();
 $('#district-select').value=state.district;studio?.setDistrict(state.district);search();sourceData('districtFocus',{type:'FeatureCollection',features:data.boundary.features.filter(f=>String(f.properties.division_code)===state.district)});
 map.fitBounds([[b[0],b[1]],[b[2],b[3]]],{padding:{top:155,bottom:125,left:35,right:35},pitch:0,bearing:0,duration:reduced?0:1000});$('#view-caption').textContent=district?.name||'南京核心六区';
 const counts=district?.counts||manifest.counts;$('#load-status').textContent=`${district?.name||'南京核心六区'} · ${fmt(counts.buildings||0)} 个建筑与部件 · OSM 开放数据`;
 });}
function renderObject(f){
 ...
 const detail=objectDetails(f,manifest);const facts=[['对象类型',kind],...detail.facts,['坐标参考','WGS84 / EPSG:4326'],['定位参考点',`${c[0].toFixed(6)} E<br>${c[1].toFixed(6)} N`],...(p.category==='building'?[['高度记录',height],['楼层记录',p.levels?`${p.levels} 层 · OSM`:'未收录']]:[]),['资料来源',p.category==='imported'?'用户本机导入':'OpenStreetMap'],['快照日期',p.category==='imported'?'本次会话':manifest.snapshotAt.slice(0,10)]];
...
function renderCatalog(){const m=manifest,c=m.counts,h=m.heights,total=c.buildings;
 ...
 $('#landmarks').innerHTML=m.landmarks.map((p,i)=>`<button class="landmark" data-place="${esc(p.id)}"><span class="place-glyph">${String(i+1).padStart(2,'0')}</span><span><strong>${esc(p.name.replace('东南大学(四牌楼校区)','东南大学 · 四牌楼').replace('南京站（南站房）','南京站'))}</strong><small>${esc(p.districtName||p.district_name||en[i]||'NANJING LANDMARK')}</small></span>${icon('arrow')}</button>`).join('');
 const rows=[['boundary','行政区边界',m.districts?.length||1,'六区 OSM 行政边界','#5e7d5c'],['buildings','地上建筑与建筑部件',c.buildings,'轮廓、标签、可用高度','#c5ae82'],['roads','道路、轨道与线状水系',c.roads,'路网分段；含相交区界对象','#c2b5a0'],['land','用地与绿地',c.land,'开放地图分类，非法定用地性质','#adc297'],['water','湖泊与水体',c.water,'面状水体及岸线','#8fbac5'],['pois','具名设施与地名',c.pois,'标签转为点位；非完整设施普查','#b7a2c3']];
 const coverage=[['OSM 高度标注',h.osm,'#c5ae82'],['楼层 × 3 m 推算',h.levels,'#97bfc6'],['高度缺失',h.unknown,'#d9d4c9']];
 $('#catalog').innerHTML=`<div class="catalog-header"><div class="section-eyebrow">EVIDENCE BEFORE INSIGHT</div><h2>每一层，都有来处。</h2><p>真实开放数据作为空间参考，业务结论建立在经过核实的项目台账之上。</p></div><div class="catalog-meta">数据快照 ${esc(m.snapshotAt)}<br>坐标 ${m.coordinateSystem}<br>许可 ODbL 1.0 · © OpenStreetMap contributors<br>${esc(m.scopeNote)}</div>${rows.map(([file,name,count,desc,color])=>`<div class="catalog-row"><i style="--c:${color}"></i><span>${name}<small>${desc}</small></span><b>${fmt(count)}</b><button data-download-layer="${file}" aria-label="下载${name}" title="下载 GeoJSON">↓</button></div>`).join('')}<div class="catalog-row"><i style="--c:#92a777"></i><span>地形高程<small>Mapzen / USGS / NOAA · 玄武片区12张 DEM 瓦片<br>地形暂限玄武周边，其他五区未补齐；非测绘级</small></span><a href="./data/terrain/metadata.json" download aria-label="下载地形元数据">↓</a></div><div class="height-coverage"><h3>建筑高度覆盖情况</h3><div class="coverage-bar" aria-hidden="true">${coverage.map(([,n,c])=>`<i style="width:${n/total*100}%;--c:${c}"></i>`).join('')}</div><div class="coverage-labels">${coverage.map(([name,n])=>`<div><span>${name}</span><b>${fmt(n)} · ${(n/total*100).toFixed(1)}%</b></div>`).join('')}</div><p class="quality-note">${esc(m.heightNote)}</p></div><div class="missing-data"><h3>真实评估仍需补齐</h3><ul>${m.missing.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div><a class="object-link" href="./data/manifest.json" download>下载数据清单与来源记录 <span>↓</span></a>`;
}
function miniMap(){const g={type:'MultiPolygon',coordinates:data.boundary.features.flatMap(f=>f.geometry.type==='Polygon'?[f.geometry.coordinates]:f.geometry.coordinates)},b=boundsOf(g),cos=Math.cos((b[1]+b[3])/2*Math.PI/180),scale=Math.min(245/((b[2]-b[0])*cos),102/(b[3]-b[1])),project=([x,y])=>[142+(x-(b[0]+b[2])/2)*cos*scale,61-(y-(b[1]+b[3])/2)*scale];
...
function search(){const q=$('#place-search').value.trim().toLowerCase();$('#clear-search').hidden=!q;$('#search-results').hidden=!q;if(!q)return;
 const found=objects.filter(f=>(state.district==='all'||String(f.properties.division_code)===state.district)&&((f.properties.name||'').toLowerCase().includes(q)||(f.properties.osm_id||'').includes(q)));
 const unique=[...new Map(found.map(f=>[`${f.properties.name}|${f.properties.category}`,f])).values()].slice(0,18);
 $('#search-results').innerHTML=unique.length?unique.map(f=>`<button data-feature="${esc(f.properties.osm_id)}"><i>⌖</i><span>${esc(f.properties.name)}<small>${esc(f.properties.district_name||'区界附近')} · ${esc(labels[f.properties.category]||'空间对象')} · ${esc(f.properties.osm_id)}</small></span>↗</button>`).join(''):'<p>本次快照中没有匹配结果。可尝试“玄武湖”“东南大学”“中山路”；未收录不代表不存在。</p>';
}
...
async function exportMap(){if(!state.ready||!map)return;$('#export-map').disabled=true;try{
 ...
 const captions=['UrbanLens · 南京核心六区 / '+(state.mode==='3d'?'3D 空间视图':'2D 空间视图'),`OSM 快照 ${manifest.snapshotAt}`,`高度：OSM 标注 / 楼层推算 / ${state.schematic?'12m 缺失示意':'缺失不拉伸'}`,'© OpenStreetMap contributors · ODbL | Terrain: Mapzen / USGS / NOAA','非测绘及法定规划成果','制作者：东南大学建筑学院 蔡子攀'];
 ...
 const blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/png'));if(!blob)throw new Error('生成图片失败');download('UrbanLens-Nanjing-'+state.mode+'.png',blob);toast('地图已导出，包含数据说明与制作者署名。');
...
async function start(){loadTimer=setTimeout(()=>fail('数据加载较慢。请检查网络后重试；已取得的数据可在资料目录中查看。'),45000);
 try{
  ...
  try{map=new maplibregl.Map({container:'map',style:makeStyle(data,state,base),center:[118.81,32.036],zoom:12.7,pitch:45,bearing:-12,minZoom:9,maxZoom:19,maxPitch:70,maxBounds:[[manifest.boundary.bbox[0]-.1,manifest.boundary.bbox[1]-.1],[manifest.boundary.bbox[2]+.1,manifest.boundary.bbox[3]+.1]],renderWorldCopies:false,attributionControl:false,localIdeographFontFamily:'-apple-system, BlinkMacSystemFont, "Helvetica Neue", Arial, "PingFang SC", "Microsoft YaHei", sans-serif',canvasContextAttributes:{antialias:true,preserveDrawingBuffer:true},fadeDuration:150});}
  catch(renderError){console.warn('WebGL unavailable; using geographic Canvas 2D.',renderError.message);state.mode='2d';state.terrain=false;$('#terrain-toggle').checked=false;$('#terrain-toggle').disabled=true;$('#pitch').disabled=true;const button=$('[data-mode="3d"]');button.disabled=true;button.title='当前浏览器未启用图形加速，已使用真实地图的 2D 兼容模式。';map=new CanvasAtlas({container:'map',style:makeStyle(data,state,base),center:[118.81,32.036],zoom:12.4,bounds:manifest.boundary.bbox,minZoom:9});$('.interaction-help').textContent='2D 兼容 · 右键详情 · 中键旋转 · Mac ⌥ 滑动';}
  ...
  map.on('load',()=>{clearTimeout(loadTimer);state.ready=true;$('#map-loading').hidden=true;$('#map-failure').hidden=true;$('#export-map').disabled=false;$('#load-status').textContent=`${map.fallback?'2D 兼容模式 · ':''}OSM ${manifest.snapshotAt.slice(0,10)} · ${fmt(manifest.counts.buildings)} 个建筑与部件`;map.getCanvas().setAttribute('aria-label','南京核心六区地图。单击选中，右键或长按查看详情；中键旋转。方向键平移，加减键缩放。');updateCamera();studio?.mapLayers();});
  Promise.all([json('data/services.geojson'),json('data/research-units.geojson'),json('data/value-evidence.json')]).then(([services,units,evidence])=>{
   studio=new ValueStudio({data,services,units,evidence,adapter:{tab,openPanel,toast,download,focus:focusFeature,layers:items=>{for(const [id,geo]of Object.entries(items))sourceData(id,geo);}}});studio.setDistrict(state.district);$('#study-hud').hidden=false;
  ...
  map.on('mousemove',e=>{if(!state.ready)return;const hit=map.queryRenderedFeatures(e.point,{layers:['research-fill','building-footprints','roads','poi-dots','station-dots','imported-fill','land'].filter(id=>map.getLayer(id))})[0],label=$('#map-hover-label');map.getCanvas().style.cursor=state.measuring?'crosshair':hit?'pointer':'';const name=hit?.properties.name;label.hidden=!name||state.measuring;if(name&&!state.measuring){label.textContent=name;label.style.left=Math.max(8,Math.min(e.point.x+16,map.getContainer().clientWidth-228))+'px';label.style.top=Math.max(10,e.point.y-43)+'px';}});
...
document.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;
 ...
 if(b.dataset.downloadLayer){const geo=data?.[b.dataset.downloadLayer];if(geo)download('UrbanLens-Nanjing-'+b.dataset.downloadLayer+'.geojson',JSON.stringify(geo));else toast('空间数据仍在加载，请稍后下载。');}
 ...
 if(b.id==='download-object'&&state.selected)download('UrbanLens-Nanjing-object.geojson',JSON.stringify({...state.selected,metadata:{snapshotAt:manifest.snapshotAt,attribution:manifest.attribution,license:manifest.license,creator:manifest.creator}},null,2));
});
$('#district-select')?.addEventListener('change',e=>setDistrict(e.target.value));
...
$('#overview').onclick=()=>setDistrict('all');
