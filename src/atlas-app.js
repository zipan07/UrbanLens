...
function miniMap(){const g=data.boundary.features[0].geometry,b=boundsOf(g),project=([x,y])=>[18+(x-b[0])/(b[2]-b[0])*245,110-(y-b[1])/(b[3]-b[1])*102];
...
function download(name,content,type='application/geo+json'){const blob=content instanceof Blob?content:new Blob([content],{type});const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=name;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}
async function exportMap(){if(!state.ready||!map)return;$('#export-map').disabled=true;try{
 await new Promise(resolve=>{map.once('render',resolve);map.triggerRepaint();});const original=map.getCanvas(),canvas=document.createElement('canvas');canvas.width=original.width;canvas.height=original.height+120;const ctx=canvas.getContext('2d');ctx.drawImage(original,0,0);ctx.fillStyle='#f8faf1';ctx.fillRect(0,original.height,canvas.width,120);ctx.fillStyle='#29483e';ctx.font='20px sans-serif';ctx.fillText('UrbanLens · 南京市玄武区 / '+(state.mode==='3d'?'3D 空间视图':'2D 空间视图'),20,original.height+30);ctx.font='12px sans-serif';ctx.fillText(`OSM 快照 ${manifest.snapshotAt} · 高度：OSM 标注 / 楼层推算 / ${state.schematic?'12m 缺失示意':'缺失不拉伸'}`,20,original.height+56);ctx.fillText('© OpenStreetMap contributors · ODbL | Terrain: Mapzen / USGS / NOAA · 非测绘及法定规划成果',20,original.height+80);ctx.fillText('制作者：东南大学建筑学院 蔡子攀',20,original.height+102);const blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/png'));if(!blob)throw new Error('生成图片失败');download('UrbanLens-Xuanwu-'+state.mode+'.png',blob);toast('地图已导出，包含数据说明与制作者署名。');
...
async function start(){loadTimer=setTimeout(()=>fail('数据加载较慢。请检查网络后重试；已取得的数据可在资料目录中查看。'),45000);
 try{
  ...
  if(!maplibregl.supported())throw new Error('此浏览器暂不支持 WebGL 地图。你仍可搜索对象、查看来源并下载数据。');
  installLocalFonts(maplibregl);maplibregl.setWorkerCount(2);
  map=new maplibregl.Map({container:'map',style:makeStyle(data,state,base),center:[118.814,32.057],zoom:14.2,pitch:52,bearing:-22,minZoom:11.8,maxZoom:19,maxPitch:70,maxBounds:[[118.735,31.995],[118.945,32.13]],renderWorldCopies:false,attributionControl:false,localIdeographFontFamily:'"Noto Sans CJK SC", "Microsoft YaHei", sans-serif',canvasContextAttributes:{antialias:true,preserveDrawingBuffer:true},fadeDuration:150});
  ...
  map.on('load',()=>{clearTimeout(loadTimer);state.ready=true;$('#map-loading').hidden=true;$('#map-failure').hidden=true;$('#export-map').disabled=false;$('#load-status').textContent=`OSM ${manifest.snapshotAt.slice(0,10)} · ${fmt(manifest.counts.buildings)} 个建筑与部件`;map.getCanvas().setAttribute('aria-label','玄武区地图。方向键平移，加减键缩放；双指缩放旋转。');updateCamera();});
 ...
 }catch(e){console.error(e);fail(e.message||'地图加载失败，请重试。');}
}
document.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;
 ...
 if(b.dataset.mode)ready(()=>{state.mode=b.dataset.mode;map.easeTo({pitch:state.mode==='2d'?0:55,bearing:state.mode==='2d'?0:map.getBearing(),duration:reduced?0:800});});
