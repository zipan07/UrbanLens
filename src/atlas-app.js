...
import {mergeDistrict,mergeServices,installDistrictChooser} from './district-data.js';
...
async function start(){loadTimer=setTimeout(()=>fail('数据加载较慢。请检查网络后重试；已取得的数据可在资料目录中查看。'),45000);
 try{
  ...
  const keys=['boundary','land','water','roads','buildings','pois'];const loaded=await Promise.all([...keys.map(k=>json(`data/${k}.geojson`)),packedBytes('gulou').then(b=>JSON.parse(new TextDecoder().decode(b)))]);data=Object.fromEntries(keys.map((k,i)=>[k,loaded[i]]));districtExtra=loaded.at(-1);const merged=mergeDistrict(data,manifest,districtExtra);manifest=merged.manifest;studyBoundary=merged.geometry;data.buildings=prepareBuildingAppearance(data.buildings);renderCatalog();
  ...
  installDistrictChooser(map,manifest,data.boundary);map.scrollZoom?.setWheelZoomRate(1/90);map.scrollZoom?.setZoomRate(1/70);map.touchZoomRotate.enable();map.touchPitch.enable();installMapGestures(map,{onInteraction:()=>shellUI?.stopOrbit()});shellUI=installStudioShell({map,manifest,studio:()=>studio,toast,tab,openPanel});map.addControl(new maplibregl.ScaleControl({maxWidth:80,unit:'metric'}),'bottom-left');
  let movementEnd;
  ...
  map.on('load',()=>{clearTimeout(loadTimer);state.ready=true;$('#map-loading').hidden=true;$('#map-failure').hidden=true;$('#export-map').disabled=false;$('#load-status').textContent=`${map.fallback?'2D 兼容模式 · ':''}OSM ${manifest.snapshotAt.slice(0,10)} · ${fmt(manifest.counts.buildings)} 个建筑与部件`;map.getCanvas().setAttribute('aria-label','南京玄武区与鼓楼区地图。单击选中，右键或长按查看详情。方向键平移，加减键缩放。');updateCamera();studio?.mapLayers();});
  Promise.all([json('data/services.geojson'),json('data/research-units.geojson'),json('data/value-evidence.json'),json('data/population-grid.geojson')]).then(([services,units,evidence,populationGrid])=>{
   services=mergeServices(services,districtExtra.data.services);
