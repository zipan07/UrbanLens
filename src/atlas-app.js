import {installCustomArea} from './custom-area.js';
...
import {installSmoothZoom} from './smooth-zoom.js';
import {mergeDistrict,mergeServices} from './district-data.js';
import {prepareBuildingAppearance} from './building-appearance.js';
import {installBuildingRealism} from './building-realism.js';
import {updateBuildingPanel} from './building-panel.js';
...
let districtExtra,studyBoundary;
let realismUI,realismManifest,map,manifest,data,studio,shellUI,detailsUI,objects=[],byId=new Map(),toastTimer,loadTimer;
...
function setBuildingRealism(enabled){state.realism=enabled;$('#realism-toggle').checked=enabled;if(enabled&&state.ready){if(state.layers.buildings===false){state.layers.buildings=true;$('[data-layer=buildings]').checked=true;applyLayerVisibility();}if(state.mode==='2d'){state.mode='3d';map.easeTo({pitch:52,duration:reduced?0:600});}if(map.getZoom()<12.4)map.easeTo({zoom:14.3,pitch:52,duration:reduced?0:600});}realismUI?.sync();}
function materialStatus(status){if(status.manifest)realismManifest=status.manifest;updateBuildingPanel(status,{map,base,enable:()=>setBuildingRealism(true)});}
...
async function json(path){if(path==='data/services.geojson')return JSON.parse(new TextDecoder().decode(await packedBytes('services')));const r=await fetch(`${base}${path}?v=16.1`,{signal:AbortSignal.timeout(30000)});if(!r.ok)throw new Error(`数据文件加载失败（${r.status}）：${path}`);return r.json();}
...
document.addEventListener('click',e=>{const a=e.target.closest('[data-export-layer]');if(a&&(a.dataset.exportLayer==='manifest'||data?.[a.dataset.exportLayer])){e.preventDefault();download('UrbanLens-Nanjing-'+a.dataset.exportLayer+'.json',JSON.stringify(a.dataset.exportLayer==='manifest'?manifest:data[a.dataset.exportLayer]));}});
function renderCatalog(){const m=manifest,c=m.counts,h=m.heights,total=c.buildings;
 ...
 $('#landmarks').innerHTML=m.landmarks.map((p,i)=>`<button class="landmark" data-place="${esc(p.id)}"><span class="place-glyph">${String(i+1).padStart(2,'0')}</span><span><strong>${esc(p.name.replace('东南大学(四牌楼校区)','东南大学 · 四牌楼').replace('南京站（南站房）','南京站'))}</strong><small>${en[i]||'GULOU / CITY LANDMARK'}</small></span>${icon('arrow')}</button>`).join('');
 const rows=[['boundary','行政区边界',data.boundary.features.length,'OSM / 320102 · 320106','#5e7d5c'],['buildings','地上建筑与建筑部件',c.buildings,'轮廓、标签、可用高度','#c5ae82'],['roads','道路、轨道与线状水系',c.roads,'路网分段；含相交区界对象','#c2b5a0'],['land','用地与绿地',c.land,'开放地图分类，非法定用地性质','#adc297'],['water','湖泊与水体',c.water,'面状水体及岸线','#8fbac5'],['pois','具名设施与地名',c.pois,'标签转为点位；非完整设施普查','#b7a2c3']];
 const coverage=[['OSM 高度标注',h.osm,'#c5ae82'],['楼层 × 3 m 推算',h.levels,'#97bfc6'],['原高度缺失 · 已推算显示',h.unknown,'#d9d4c9']];
 $('#catalog').innerHTML=`<div class="catalog-header"><div class="section-eyebrow">EVIDENCE BEFORE INSIGHT</div><h2>每一层，都有来处。</h2><p>真实开放数据作为空间参考，业务结论建立在经过核实的项目台账之上。</p></div><div class="catalog-meta">数据快照 ${esc(m.snapshotAt)}<br>坐标 ${m.coordinateSystem}<br>许可 ODbL 1.0 · © OpenStreetMap contributors<br>${esc(m.scopeNote)}</div>${rows.map(([file,name,count,desc,color])=>`<div class="catalog-row"><i style="--c:${color}"></i><span>${name}<small>${desc}</small></span><b>${fmt(count)}</b><a href="./data/${file}.geojson" data-export-layer="${file}" download="UrbanLens-Nanjing-${file}.geojson" aria-label="下载${name}" title="下载 GeoJSON">↓</a></div>`).join('')}<div class="catalog-row"><i style="--c:#92a777"></i><span>地形高程<small>Mapzen / USGS / NOAA · 12 张区域 DEM 瓦片<br>真实地形网格，非测绘级；原始采集年份不一</small></span><a href="./data/terrain/metadata.json" download aria-label="下载地形元数据">↓</a></div><div class="height-coverage"><h3>建筑高度覆盖情况</h3><div class="coverage-bar" aria-hidden="true">${coverage.map(([,n,c])=>`<i style="width:${n/total*100}%;--c:${c}"></i>`).join('')}</div><div class="coverage-labels">${coverage.map(([name,n])=>`<div><span>${name}</span><b>${fmt(n)} · ${(n/total*100).toFixed(1)}%</b></div>`).join('')}</div><p class="quality-note">${esc(m.heightNote)}</p></div><div class="missing-data"><h3>真实评估仍需补齐</h3><ul>${m.missing.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div><a class="object-link" href="./data/manifest.json" data-export-layer="manifest" download>下载三区数据清单与来源记录 <span>↓</span></a>`;
}
function miniMap(){const g=studyBoundary||data.boundary.features[0].geometry,b=boundsOf(g),cos=Math.cos((b[1]+b[3])/2*Math.PI/180),scale=Math.min(245/((b[2]-b[0])*cos),102/(b[3]-b[1])),project=([x,y])=>[142+(x-(b[0]+b[2])/2)*cos*scale,61-(y-(b[1]+b[3])/2)*scale];
...
async function exportMap(){if(!state.ready||!map)return;$('#export-map').disabled=true;try{
 ...
 const captions=['UrbanLens · 南京市玄武区、鼓楼区与秦淮区 / '+(state.mode==='3d'?'3D 空间视图':'2D 空间视图'),`OSM 快照 ${manifest.snapshotAt}`,`高度：OSM 标注 / 楼层推算 / ${state.schematic?'类型 / 轮廓 / 邻近高度推算':'缺失不拉伸'}`,'© OpenStreetMap contributors · ODbL | Terrain: Mapzen / USGS / NOAA',`人口：${state.population==='none'?'未显示':state.population==='day'?'白天情景推算':'夜间居住推算'}；${state.realism?'实景立面 / Poly Haven CC0 材质；体量细节示意，署名见图层来源':'基础体量'}`,'非测绘及法定规划成果','制作者：蔡子攀｜东南大学建筑学院 · 东南大学城市规划设计研究院'];
 if(state.realism&&realismManifest){captions.push('照片仅作局部立面贴图，经过透视校正和裁切；照片改编沿用原 CC 许可。');for(const m of realismManifest.materials.filter(m=>m.type==='photo'))captions.push(`${m.name} · ${m.author} · ${m.license} | ${m.sourcePage} | ${m.licenseUrl}`);captions.push('扫描材质：Poly Haven · CC0 | https://polyhaven.com/license');}
...
async function start(){loadTimer=setTimeout(()=>fail('数据加载较慢。请检查网络后重试；已取得的数据可在资料目录中查看。'),45000);
 try{
  ...
  const keys=['boundary','land','water','roads','buildings','pois'];const loaded=await Promise.all([...keys.map(k=>json(`data/${k}.geojson`)),...['gulou','qinhuai'].map(name=>packedBytes(name).then(b=>JSON.parse(new TextDecoder().decode(b))))]);data=Object.fromEntries(keys.map((k,i)=>[k,loaded[i]]));districtExtra=loaded.slice(keys.length);for(const extra of districtExtra){const merged=mergeDistrict(data,manifest,extra);manifest=merged.manifest;studyBoundary=merged.geometry;}data.buildings=prepareBuildingAppearance(data.buildings);renderCatalog();
  ...
  catch(renderError){console.warn('WebGL unavailable; using geographic Canvas 2D.',renderError.message);state.mode='2d';state.terrain=false;$('#terrain-toggle').checked=false;$('#terrain-toggle').disabled=true;$('#pitch').disabled=true;const button=$('[data-mode="3d"]');button.disabled=true;button.title='当前浏览器未启用图形加速，已使用真实地图的 2D 兼容模式。';map=new CanvasAtlas({container:'map',style:makeStyle(data,state,base),center:[118.811,32.059],zoom:13.4,bounds:manifest.boundary.bbox,minZoom:10.5});$('.interaction-help').textContent='2D 兼容 · 右键详情 · 中键平移 · Mac ⌥ 滑动';}
  realismUI=installBuildingRealism({map,base,buildings:data.buildings,getState:()=>state,onStatus:materialStatus,onError:error=>{setBuildingRealism(false);toast('精细建筑暂不可用，已保留基础建筑');console.warn(error.message);}});if(map.fallback){$('#realism-toggle').disabled=true;$('#realism-toggle').title='外立面与屋顶需要 3D 图形加速';}
  installMapGestures(map,{onInteraction:()=>shellUI?.stopOrbit()});const zoomControls=installSmoothZoom(map,{onInteraction:()=>shellUI?.stopOrbit()});$('#zoom-in').onclick=()=>ready(()=>zoomControls.step(.65));$('#zoom-out').onclick=()=>ready(()=>zoomControls.step(-.65));map.touchZoomRotate.enable();map.touchPitch.enable();shellUI=installStudioShell({map,manifest,studio:()=>studio,toast,tab,openPanel});map.addControl(new maplibregl.ScaleControl({maxWidth:80,unit:'metric'}),'bottom-left');
  let movementEnd;
  ...
  map.on('load',()=>{clearTimeout(loadTimer);state.ready=true;$('#map-loading').hidden=true;$('#map-failure').hidden=true;$('#export-map').disabled=false;$('#load-status').textContent=`${map.fallback?'2D 兼容模式 · ':''}OSM ${manifest.snapshotAt.slice(0,10)} · ${fmt(manifest.counts.buildings)} 个建筑与部件`;map.getCanvas().setAttribute('aria-label','南京玄武区、鼓楼区与秦淮区地图。单击选中，右键或长按查看详情。方向键平移，加减键缩放。');updateCamera();studio?.mapLayers();});
  Promise.all([json('data/services.geojson'),json('data/research-units.geojson'),json('data/value-evidence.json'),json('data/population-grid.geojson')]).then(([services,units,evidence,populationGrid])=>{
   for(const extra of districtExtra)services=mergeServices(services,extra.data.services);
   data.populationGrid=populationGrid;applyPopulation();
   const drawing=installCustomArea({map,boundary:studyBoundary,toast,onComplete:feature=>studio.acceptDrawnArea(feature),onActive:active=>{document.body.classList.toggle('is-drawing',active);if(active){shellUI?.stopOrbit();detailsUI?.close();$('#map-hover-label').hidden=true;if(state.measuring)$('#measure').click();openPanel(false);}}});
   studio=new ValueStudio({data,services,units,evidence,adapter:{studyBoundary,population:mode=>{state.population=mode;applyPopulation();},drawArea:mode=>ready(()=>drawing.start(mode)),cancelDrawing:()=>drawing.cancel(),tab,openPanel,toast,download,focus:focusFeature,getViewport,resetView,showObject:f=>select(f,{fly:true}),clearObjectSelection,layers:items=>{for(const [id,geo]of Object.entries(items))sourceData(id,geo);const visible=items.research?.features;if(visible&&state.selected&&(state.selected.properties.unit_id||state.selected.properties.category==='imported')&&!visible.some(f=>f.properties.unit_id===(state.selected.properties.unit_id||state.selected.properties.parcel_id)))clearObjectSelection();}}});new ProjectWorkspace(studio);refreshImportedSource();
  ...
  const layerSources={'building-realism-pick':'buildings','building-3d':'buildings','building-footprints':'buildings',roads:'roads',tunnels:'roads',rail:'roads',waterways:'roads','poi-dots':'pois','station-dots':'pois',land:'land',water:'water'},indexes=Object.fromEntries(Object.keys(data).map(k=>[k,new Map(data[k].features.map(f=>[f.properties.osm_id,f]))]));
  ...
  detailsUI=installMapDetails({map,manifest,pick,isReady:()=>state.ready&&!state.measuring&&!document.body.classList.contains('is-drawing'),toast,
  ...
  map.on('click',e=>{if(!state.ready||document.body.classList.contains('is-drawing')||detailsUI.consumeLongPress())return;detailsUI.close();if(state.measuring){state.measure.push([e.lngLat.lng,e.lngLat.lat]);updateMeasure();return;}
  ...
  map.on('mousemove',e=>{if(!state.ready||document.body.classList.contains('is-drawing'))return;const hit=map.queryRenderedFeatures(e.point,{layers:['research-fill','building-realism-pick','building-3d','poi-dots','station-dots','imported-fill'].filter(id=>map.getLayer(id))})[0],label=$('#map-hover-label');map.getCanvas().style.cursor=state.measuring?'crosshair':hit?'pointer':'';const name=hit?.properties.name;label.hidden=!name||state.measuring;if(name&&!state.measuring){label.textContent=name;label.style.left=Math.max(8,Math.min(e.point.x+16,map.getContainer().clientWidth-228))+'px';label.style.top=Math.max(10,e.point.y-43)+'px';}});
...
$('#realism-toggle').onchange=e=>setBuildingRealism(e.target.checked);
