...
function customLayer(getState,mesh,pack){return {id:REALISM_LAYER_ID,type:'custom',renderingMode:'3d',mesh,pack,
 onAdd(map,gl){this.map=map;this.gl=gl;let vs,fs;try{
  ...
  this.buffer=gl.createBuffer();this.vao=gl.createVertexArray();gl.bindVertexArray(this.vao);gl.bindBuffer(gl.ARRAY_BUFFER,this.buffer);let offset=0;[3,3,3,3,1,1,1,4].forEach((size,i)=>{gl.enableVertexAttribArray(i);gl.vertexAttribPointer(i,size,gl.FLOAT,false,76,offset*4);offset+=size;});gl.bindVertexArray(null);
 ...
 render(gl,options){const state=getState();if(!state.realism||state.mode==='2d'||state.layers?.buildings===false||this.map.getPitch()<2||this.map.getZoom()<REALISM_LIMITS.minZoom||!this.program)return;const matrix=options.defaultProjectionData?.mainMatrix;if(!matrix)return;
  ...
  gl.depthMask(true);gl.disable(gl.BLEND);gl.drawArrays(gl.TRIANGLES,shadows,this.mesh.vertices.length/this.mesh.stride-shadows);gl.disable(gl.POLYGON_OFFSET_FILL);gl.bindVertexArray(null);this.textures.forEach((_,i)=>{gl.activeTexture(gl.TEXTURE0+i);gl.bindTexture(gl.TEXTURE_2D_ARRAY,null);});gl.activeTexture(gl.TEXTURE0);
 },
...
export function installBuildingRealism({map,buildings,getState,base=null,onStatus=()=>{},onError=()=>{}}){
 ...
 function sync(){if(destroyed)return;clearTimeout(timer);timer=null;const state=getState(),active=state.realism&&state.mode!=='2d'&&state.layers?.buildings!==false&&map.getPitch()>=2&&map.getZoom()>=REALISM_LIMITS.minZoom;
  ...
  try{
   const bounds=map.getBounds(),c=map.getCenter(),west=bounds.getWest(),south=bounds.getSouth(),east=bounds.getEast(),north=bounds.getNorth(),detail=map.getZoom()>=16.2;
