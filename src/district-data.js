import {installDistrictFlow} from './district-flow.js';
...
export function installDistrictChooser(map,manifest,boundaries){
 const flow=installDistrictFlow(map,boundaries);
 ...
 document.querySelector('.map-workspace').append(group);flow.highlight(manifest.districts[0].name);
 group.querySelector('select').addEventListener('change',e=>{flow.highlight(e.target.value==='all'?null:manifest.districts[Number(e.target.value)].name);const b=e.target.value==='all'?manifest.boundary.bbox:manifest.districts[Number(e.target.value)].bbox;map.fitBounds([[b[0],b[1]],[b[2],b[3]]],{padding:45,pitch:map.fallback?0:map.getPitch(),duration:matchMedia('(prefers-reduced-motion: reduce)').matches?0:1000});});
}
