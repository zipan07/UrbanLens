...
import {heightOf,centerOf,boundsOf,inGeometry,coordinates} from '../src/geo.js';

const rawDir=process.argv[2]||'/tmp';
const load=name=>JSON.parse(fs.readFileSync(`${rawDir}/qinhuai-${name}-raw.json`));
const boundary=osm(JSON.parse(fs.readFileSync(`${rawDir}/../qinhuai-osm-response.json`)),{flatProperties:true}).features.find(f=>f.id==='relation/2140011');
if(!boundary)throw new Error('秦淮区行政边界缺失');
const raw=load('bbox');if(raw.remark)throw new Error(raw.remark);
const extra={elements:[]};
const converted=osm({...raw,elements:[...raw.elements,...extra.elements]},{flatProperties:true});
const fc=features=>({type:'FeatureCollection',metadata:{source:'OpenStreetMap',attribution:'© OpenStreetMap contributors',license:'ODbL 1.0',licenseUrl:'https://opendatacommons.org/licenses/odbl/1-0/',snapshotAt:raw.osm3s.timestamp_osm_base,scope:'Qinhuai District bounding-box query filtered by district geometry; complete intersecting objects; not a cadastral survey'},features});
...
function feature(f,extra={}){const p=f.properties;return {...f,properties:{...props(p),district_name:'秦淮区',source_date:raw.osm3s.timestamp_osm_base,osm_id:f.id,label:(p.name||'').replace(/[^\p{Script=Han}]/gu,''),...extra}};}
for(const f of converted.features){
  ...
  if(!coordinates(f.geometry).some(p=>inGeometry(p,boundary.geometry))&&!inGeometry(centerOf(f.geometry),boundary.geometry)&&!(['Polygon','MultiPolygon'].includes(f.geometry.type)&&inGeometry(centerOf(boundary.geometry),f.geometry)))continue;
...
const landmarks=groups.pois.filter(f=>['way/590937998','way/318052307','way/948332291'].includes(f.id)).map(f=>({id:f.id,name:f.properties.name,center:centerOf(f.geometry),bbox:boundsOf(f.geometry)}));
const b=feature(boundary,{category:'boundary',division_code:'320104'});
const out=process.argv[3]||'dist/data/qinhuai';fs.mkdirSync(out,{recursive:true});
...
const manifest={version:'QINHUAI-22',title:'南京市秦淮区',divisionCode:'320104',coordinateSystem:'WGS84 / EPSG:4326',snapshotAt:raw.osm3s.timestamp_osm_base,areaSnapshotAt:raw.osm3s.timestamp_areas_base,downloadedAt:new Date().toISOString().slice(0,10),boundary:{osmId:boundary.id,bbox:boundsOf(boundary.geometry)},counts:Object.fromEntries(Object.entries(groups).map(([k,v])=>[k,v.length])),heights:h,skipped,landmarks,license:'ODbL 1.0',attribution:'© OpenStreetMap contributors',sourceUrl:'https://www.openstreetmap.org/relation/2140011',queryEndpoint:'https://overpass-api.de/api/interpreter',scopeNote:'OSM 秦淮区外包范围检索，按顶点或中心落区筛选，保留完整对象；边界附近可能存在遗漏或跨区对象，非城区普查总量。',heightNote:'height 为 OSM 标注，未经测绘复核；building:levels × 3 m 为推算；无高度记录时仅显示 12 m 示意体量，可关闭。所有显示高度均不参与真实用地评估。',missing:['权属与宗地台账','法定规划条件与控制线','建筑调查与安全鉴定','空置率与运营调查','授权正射影像与实景三维模型'],creator:'制作者：蔡子攀｜东南大学建筑学院 · 东南大学城市规划设计研究院'};
