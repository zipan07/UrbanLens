...
const folder=process.argv[2]||'dist/data/gulou',rawFile=process.argv[3];
...
for(const f of osm(raw,{flatProperties:true}).features){const p=f.properties,c=category(p);if(!c||!f.geometry||p.tainted||p.disused==='yes'||p.abandoned==='yes'||p.proposed==='yes'||p.construction||p.railway==='construction'||p.access==='no')continue;const xy=centerOf(f.geometry);if(!inGeometry(xy,boundary))continue;
 ...
 points.push({type:'Feature',id:f.id,geometry:{type:'Point',coordinates:xy},properties:{osm_id:f.id,name:p.name||'未命名设施',named:!!p.name,category:c,kind:p.amenity||p.shop||p.railway||p.highway||p.leisure||p.historic,location_method:'OSM 节点或对象外包框中心，非入口',source_date:manifest.snapshotAt,related_osm_ids:[f.id],district:'鼓楼区'}});
}
data.services={type:'FeatureCollection',metadata:{source:manifest.queryEndpoint,snapshotAt:manifest.snapshotAt,license:'ODbL 1.0',note:'鼓楼区中心点落区筛选；非设施普查。同类同名80米以内合并。'},features:points};
...
for(let i=0;i<base64.length;i+=90000){const name=`gulou-${hash}-${String(parts.length+1).padStart(2,'0')}.txt`;fs.writeFileSync('dist/packed/'+name,base64.slice(i,i+90000));parts.push(name);}
fs.writeFileSync('dist/packed/gulou.json',JSON.stringify({encoding:'gzip-base64',bytes:bytes.length,parts}));
fs.writeFileSync('dist/data/gulou-manifest.json',JSON.stringify({...manifest,services:points.length,pack:'../packed/gulou.json'},null,2));
